...
- For passages: rotate through a wide range of topics. Do NOT default to redwood trees, monarch butterflies, or condors repeatedly.
...
function CanvaView({ parsed, subject, rawText }) {
  ...
  const canvaSections = [];
  canvaSections.push({ title:"① HEADER BLOCK — paste into the top colored header", content:`${parsed.title}\n${parsed.subtitle}` });
  canvaSections.push({ title:"② NAME / DATE / SCORE LINE — paste below the header", content:"Name: ________________________     Date: ___________     Score: ______" });
  ...
  parsed.sections.forEach((sec, i) => {
    ...
    const typeHint = {multiple_choice:"MULTIPLE CHOICE — use bubble answer choices below each question",word_problem:"WORD PROBLEMS — add a 'Show Your Work' box below each",short_answer:"SHORT ANSWER — add 2–3 answer lines below each question",explain:"EXPLAIN THINKING — add 4 answer lines below the question"}[sec.type] || sec.type.toUpperCase();
    canvaSections.push({ title:`${circle} SECTION: ${sec.heading.toUpperCase()} — ${typeHint}`, content:sec.content });
  });
  if (parsed.bonus) canvaSections.push({ title:"⑪ BONUS CHALLENGE BOX — use a highlighted or star-bordered box", content:`⭐ BONUS CHALLENGE\n${parsed.bonus}` });
  canvaSections.push({ title:"⑫ FOOTER — small text at bottom of page", content:`© ${new Date().getFullYear()} · 5th Grade · ${subject.label} · For classroom and TPT use` });
  if (parsed.answerKey) canvaSections.push({ title:"⑬ ANSWER KEY PAGE — duplicate your worksheet page, delete questions, paste this", content:`ANSWER KEY\n${parsed.title}\n\n${parsed.answerKey}` });
  if (parsed.teacherNotes) canvaSections.push({ title:"⑭ TEACHER NOTES — optional back page or TPT product description", content:parsed.teacherNotes });
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        ...
        <div className="mt-4 flex items-center gap-3 bg-slate-50 rounded-xl p-3">
          ...
          <div><p className="text-xs font-bold text-slate-600">Your subject color</p><p className="text-xs text-slate-400">In Canva: click header → change color → paste <span className="font-mono font-bold text-slate-700">{hc}</span></p></div>
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl px-5 py-4 flex items-start gap-3">
        ...
        <div><p className="text-xs font-bold text-indigo-700 mb-1">TPT Seller Tip</p><p className="text-xs text-indigo-600 leading-relaxed">Generate 3–5 variations of the same resource type (e.g. 5 different Fractions Practice sheets). Bundle them into one Canva file with a cover page. That's a complete TPT product. Price point: $3–5 for a 5-pack. Cover page = your thumbnail.</p></div>
      </div>
    </div>
  );
}
...
export default function RexStudio() {
  ...
  const handleSubject = (idx) => { const s=SUBJECTS[idx]; setSubjectIdx(idx); setResourceType(Object.keys(STANDARDS[s.id])[0]); setRawText(""); setParsed(null); setError(""); };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-amber-50">
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-3.5 flex items-center justify-between sticky top-0 z-10 shadow-sm rex-no-print">
        <div className="flex items-center gap-3">
          ...
          <div><div className="font-extrabold text-slate-800 text-sm tracking-wide">REX</div><div className="text-xs text-slate-400">Resource Studio · 5th Grade CA</div></div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-5 rex-no-print">
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm flex items-start gap-2">
              ...
              <div><p>{error}</p><button onClick={generate} className="mt-2 flex items-center gap-1.5 text-xs font-bold bg-red-100 hover:bg-red-200 px-3 py-1.5 rounded-lg transition-all"><RefreshCw size={11}/> Try Again</button></div>
            </div>
          )}
        </div>
